...
walk(species, function(tax_id) {
    ...
    function(gene_mesh) {
        ...
        if (file.exists(gene_mesh_matrix_rds) &&
            file.exists(gene_mesh_tf_idf_rds)) {
            return(NULL)
        }

        print(gene_mesh_file)
        dat <- read_tsv(
            file.path(main_path, gene_mesh_file),
        ...
        dat_matrix = sparseMatrix(
            i = dat %>% pull(gene_id) %>% match(dat_gene),
